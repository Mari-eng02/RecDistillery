...
def _row_from_split(payload: dict, split: str, context: dict[str, str]) -> dict[str, str]:
    ...
    return {
        **context,
        ...
        "distiller": _safe_str(payload.get("distiller")),
...
def write_tsv(rows: list[dict[str, str]], output_path: Path) -> None:
    ...
    if not rows:
        with output_path.open("w", encoding="utf-8") as fp:
            fp.write("dataset\tstudent\tphase\teval_json_file\n")
        return
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Aggregate baseline student evaluation metrics into one TSV.")
    parser.add_argument("--results-root", default="results", help="Root results directory")
    parser.add_argument(
        "--output",
        default="results/student_eval_perf_aggregated.tsv",
