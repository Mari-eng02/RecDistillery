...
usage() {
  echo "Usage: bash experiments/baseline/student_training_cite.sh [gpu]"
  echo
  ...
  echo "  bash experiments/baseline/student_training_cite.sh 0"
}
...
mkdir -p "${LOG_DIR}"
RUNNER_LOG="${LOG_DIR}/student_training_cite_${DATASET}.log"

if [ "${INTERNAL_RUN}" != "--internal-run" ]; then
  nohup bash experiments/baseline/student_training_cite.sh --internal-run "${GPU}" \
