...
usage() {
  echo "Usage: bash experiments/baseline/student_training_cite.sh [gpu] [grid|best]"
  echo
  echo "Runs student_training.py sequentially on dataset 'citeulike'"
  echo "for the models: BPRMF, LGCN, NMF."
  echo "The launcher detaches automatically, so the terminal can be closed."
  echo
  echo "Examples:"
  echo "  bash experiments/baseline/student_training_cite.sh"
  echo "  bash experiments/baseline/student_training_cite.sh 0 grid"
  echo "  bash experiments/baseline/student_training_cite.sh 1 best"
}
...
mkdir -p "${LOG_DIR}"
RUNNER_LOG="${LOG_DIR}/student_training_cite_${DATASET}_${BEST_FLAG}.log"

if [ "${INTERNAL_RUN}" != "--internal-run" ]; then
  nohup bash experiments/baseline/student_training_cite.sh --internal-run "${GPU}" "${BEST_FLAG}" \
