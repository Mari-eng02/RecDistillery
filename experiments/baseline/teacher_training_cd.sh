...
usage() {
  echo "Usage: bash experiments/baseline/teacher_training_cd.sh [gpu] [grid|best]"
  echo
  ...
  echo "  bash experiments/baseline/teacher_training_cd.sh 0 grid"
  echo "  bash experiments/baseline/teacher_training_cd.sh 1 best"
}
...
mkdir -p "${LOG_DIR}"
RUNNER_LOG="${LOG_DIR}/teacher_training_cd_${DATASET}_${BEST_FLAG}.log"

if [ "${INTERNAL_RUN}" != "--internal-run" ]; then
  nohup bash experiments/baseline/teacher_training_cd.sh --internal-run "${GPU}" "${BEST_FLAG}" \
