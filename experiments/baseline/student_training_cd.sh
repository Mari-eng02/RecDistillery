...
usage() {
  echo "Usage: bash experiments/baseline/student_training_cd.sh [gpu]"
  echo
  ...
  echo "  bash experiments/baseline/student_training_cd.sh 0"
}
...
mkdir -p "${LOG_DIR}"
RUNNER_LOG="${LOG_DIR}/student_training_cd_${DATASET}.log"

if [ "${INTERNAL_RUN}" != "--internal-run" ]; then
  nohup bash experiments/baseline/student_training_cd.sh --internal-run "${GPU}" \
