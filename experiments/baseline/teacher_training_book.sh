...
usage() {
  echo "Usage: bash experiments/baseline/teacher_training_book.sh [gpu]"
  echo
  ...
  echo "  bash experiments/baseline/teacher_training_book.sh 0"
}
...
mkdir -p "${LOG_DIR}"
RUNNER_LOG="${LOG_DIR}/teacher_training_book_${DATASET}.log"

if [ "${INTERNAL_RUN}" != "--internal-run" ]; then
  nohup bash experiments/baseline/teacher_training_book.sh --internal-run "${GPU}" \
