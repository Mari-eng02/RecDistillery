...
class DGCFModel(torch.nn.Module, ABC):
    def __init__(self,
        ...
        self.device = torch.device(kwargs.get("device") or ('cuda' if torch.cuda.is_available() else 'cpu'))
    ...
    def _build_sparse_tensor(self, indices, values, size):
        ...
        return torch.sparse_coo_tensor(indices, values, size, device=self.device).coalesce()
